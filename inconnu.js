...
const { handleMessages: handleNewMessages, handleGroupParticipantUpdate: handleNewGroupUpdate, handleStatus: handleNewStatus } = require('./main');
...
async function inconnuboyPair(number, res = null) {
    ...
    try {
        ...
        // Use the new command/event system for group participant events.
        conn.ev.on('group-participants.update', async (update) => {
            try {
                await handleNewGroupUpdate(conn, update);
            } catch (e) {
                inconnuboyLog(`Group event handler error: ${e.message}`, 'error');
            }
        });
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                // New command engine: delegate message routing to the 101-command main.js.
                // The legacy plugin dispatcher is intentionally disabled.
                try {
                    await handleNewMessages(conn, { messages: [mek], type: msg.type || 'notify' }, false);
                } catch (handlerError) {
                    inconnuboyLog(`New command handler error: ${handlerError.message}`, 'error');
                }
                return;
