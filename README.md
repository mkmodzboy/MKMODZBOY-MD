...
### No manual session ID or `creds.json` upload is required

Use the web pairing page after deployment. Enter the WhatsApp number, generate the pairing code, then link the device from WhatsApp → Linked Devices. The server creates and stores the per-number session automatically.
